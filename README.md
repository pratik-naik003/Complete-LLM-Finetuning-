...
# 📘 Evaluation Metrics (Simple Notes)

This document explains important NLP evaluation metrics in **very simple English** with examples and code.

---

# 1. Accuracy (Basic Idea)
...
* Predictions: [0, 1, 1, 0]
* Actual (Reference): [0, 1, 0, 0]

Correct = 3 out of 4 → Accuracy = 75%

---

## ✅ Code Example (Hugging Face Evaluate)
...
# 2. Confusion Matrix (Basic Understanding)

It helps to understand predictions better.
...
# 3. BLEU Score (Precision-based)

👉 Used for:

* Machine Translation
* Text Generation (less ideal)

👉 Idea:
It checks how much **predicted text matches reference text**.
...
Reference: "Sunny is a boy"
Prediction: "Sunny is a man"

* Unigram match: Sunny, is → match
* Bigram match: "Sunny is" → match
* "is boy" vs "is man" → mismatch

---

## ❗ Important Rule

If any higher gram (like 4-gram) = 0 → Final BLEU score becomes 0
...
* BLEU score
* Precision values (1g, 2g, 3g, 4g)
* Brevity penalty
* Length ratio

---

# 4. ROUGE Score (Recall-based)
...
👉 Idea:
Checks how much of **reference text is covered by prediction**
...
| Matching | Exact n-grams | Flexible (LCS) |

---

# 5. Perplexity (Most Important for LLMs)
...
👉 Idea:
Measures **how confused the model is** when predicting next word.
...
👉 Low confusion → Low Perplexity → GOOD model
...
👉 Model confused → High Perplexity → BAD model
...
* BLEU → Good for translation
* ROUGE → Good for summarization
* Perplexity → Best for text generation models
...
# 📘 Hugging Face API + LangChain (Simple Notes)

---

## 🔹 1. What is Hugging Face API?

Hugging Face API helps you:

* Load models directly from Hugging Face Hub
* Do prediction (inference) without downloading models
...
* `pipeline()` → downloads model in memory
* `snapshot_download()` → downloads model on disk

### 👉 Now:

* HF API → No need to download, directly use model from server

---

## 🔹 2. Create Hugging Face API Object
...
## 🔹 4. List Models with Filters

### Example: Emotion Detection Models

```python
models = api.list_models(filter="emotion", sort="downloads", limit=5)
...
* `filter` → type of task (emotion, text-generation, etc.)
...
```python
repo_files = api.list_repo_files("google/flan-t5-base")
print(repo_files)
```

👉 Shows all files inside model repo
...
👉 Gives dataset details like files and structure

---

## 🔹 7. Download File from Hugging Face
...
👉 Downloads specific file (like config.json)

---

## 🔹 8. Inference Without Downloading Model
...
👉 Model runs on Hugging Face server (no local download)
...
👉 Gives top text generation models

---

## 🔹 10. Important Issue (Very Important)

### ⚠️ Problem:

* Some models don’t work due to provider mismatch

### ✅ Solution:

1. Downgrade huggingface_hub version
2. Check supported providers
3. Use correct task (text-generation, chat, etc.)
...
```bash
pip install langchain huggingface_hub langchain-community langchain-huggingface
```
...
## 🔹 13. Load Model via Endpoint (No Download)
...
```python
response = chat_model.invoke("Who is the first president of India?")
print(response)
```

---

## 🔹 15. Prompt Template + Chain
...
prompt = PromptTemplate(
    ...
    template="Answer in detail step by step: {question}"
)
...
result = chain.invoke("Who is the first president of India?")
print(result)
```

👉 This creates full pipeline:

* Input → Prompt → Model → Output

---

## 🔹 16. Hugging Face Pipeline (Local Model)

👉 This method downloads model locally
...
* Large models need **quantization (bitsandbytes)**
...
* Hugging Face API is used for direct model access
* You can filter and search models easily
* Use InferenceClient for quick predictions
* LangChain helps build powerful AI apps
* Always check provider compatibility
...
* Load models
* Run inference
* Build AI pipelines using LangChain
